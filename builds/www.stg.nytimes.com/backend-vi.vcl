sub vcl_recv {
  ...
  if (   req.url ~ "^/2(01[4-9]|(0[2-9][0-9])|([1-9][0-9][0-9]))" // 2014-future
      ...
      if (
        (req.url ~ "^/2016/1[0-2]" || req.url ~ "^/201[7-9]") // October 2016 and after
        && req.http.x--fastly-vi-test-group-story ~ "^[a]"
          ...
          if (req.http.x-pre-restart-status == "Incompatible") {
            ...
            call check_vi_unhealthy_article;
          }
      } else {
  ...
  if (req.url ~ "^/interactive/20(1[4-9]|[2-9][0-9])/") {
    ...
    &&  req.url.path ~ "\.(embedded|mobile|app)\.html$") {
      ...
      call set_www_fe_backend;
        ...
        call set_projectvi_fe_backend;
        call check_vi_unhealthy_interactive;
    }
  }
      ...
      if (req.http.host ~ "^(www\.)?(www-[a-z0-9\-]+\.)?(dev\.|stg\.|)?nytimes.com$") {
          if ( req.url.path == "/"
            && ((req.http.x--fastly-vi-test-group ~ "^[ab]" && req.http.cookie:vi_www_hp_opt != "0") || req.http.cookie:vi_www_hp_opt == "1")
              ...
              call set_projectvi_fe_backend;
              call check_vi_unhealthy_homepage;
...
sub vcl_hash {
  ...
  if (req.http.x--fastly-project-vi){
    ...
    } else if ( req.url ~ "^/2016/1[0-2]" || req.url ~ "^/201[7-9]"){
...
sub check_vi_unhealthy_article {
    if (
...
sub check_vi_unhealthy_homepage {
    if (
        (req.backend == projectvi_fe_prd || req.backend == projectvi_fe_stg)
        && !req.backend.healthy
    ) {
        set req.http.X-PageType = "homepage";
        call handle_vi_unhealthy;
    }
}

sub check_vi_unhealthy_interactive {
    if (
        (req.backend == projectvi_fe_prd || req.backend == projectvi_fe_stg)
        && !req.backend.healthy
    ) {
