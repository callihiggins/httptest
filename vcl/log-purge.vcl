sub recv_sumologic_purge_log_line {
  ...
  log {"syslog "} req.service_id {" "} req.http.var-nyt-sumo-log-name {" :: "}
  {"  {  "client_ip":""} req.http.Fastly-Client-IP {"", "protocol":""} if(req.http.Fastly-SSL,"https","http") {"", "host":""} cstr_escape(req.http.host) {"", "request":""} req.request {"",  "soft":""} cstr_escape(req.http.fastly-soft-purge) {"",  "key":""} cstr_escape(req.http.fastly-key) {"",  "url":""} cstr_escape(req.url) {"",  "request_user_agent":""} cstr_escape(req.http.user-agent) {"", "route":""} if(req.http.x-nyt-route,req.http.x-nyt-route,"") {""  }   "};
